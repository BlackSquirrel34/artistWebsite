...
export default function SpaceGridGallery() {
  ...
  const imagesWithSpans = images.map((img) => {
    ...
    const rowSpan = Math.max(1, Math.round((1 / aspectRatio) * 2)) // Adjust as needed
      ...
      >
        {imagesWithSpans.map((img, index) => (
          <div
            ...
            className="overflow-hidden rounded flex items-center justify-center p-1" // added padding for whitespace
          >
            <img
              src={img.url.src}
              alt={img.alt}
              className="max-w-full max-h-full object-contain" // preserve aspect ratio, fit within container
            />
          </div>
        ))}
      </div>
